...
export default function Connection() {
  ...
  const { data: connections, isLoading } = useSWR(
    user
      ? `/connections/me?_limit=-1&status=ignored&authorProfile=${user?.profile?.id}`
      : null,
    sessionGet
  );
                    ...
                    <div className="px-4 py-4 sm:px-6 md:flex md:items-center md:justify-between md:space-x-5">
                      ...
                      <div className="justify-stretch mt-6 flex flex-col-reverse space-y-4 space-y-reverse md:mt-0 md:flex-row md:space-x-3">
                        <button
                          ...
                          className="inline-flex items-center justify-center rounded-md border border-transparent bg-blue-600 px-4 py-1.5 text-sm font-medium text-white shadow-sm hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 focus:ring-offset-gray-100"
                        >
                          Accept
                        </button>
                      </div>
