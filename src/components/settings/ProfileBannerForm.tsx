...
export default function ProfileBannerForm({ user, token }: IPageProps) {
  ...
  const uploadProfilePicture = async () => {
    ...
    try {
      if (imageRef.current && crop.width && crop.height) {
        ...
        body.append('field', 'profileBanner');
    ...
    <div className="col-span-3">
      <div className="mt-1 flex items-center">
        <span className="inline-block overflow-hidden bg-gray-100">
          {user?.profile?.profileBanner?.url ? (
            <img
              alt={`${user?.profile?.firstName} ${user?.profile?.lastName}`}
              src={user?.profile?.profileBanner?.url}
              className=""
            />
          ) : (
            <svg
        ...
        <div className="space-y-6 bg-white py-6 px-4 sm:p-6">
          <div>
            ...
            <p className="mt-1 text-sm text-gray-500">
              Choose banner for your profile
            </p>
          </div>
